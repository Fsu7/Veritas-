...
flowchart TB
    subgraph 独创性["🎯 7 大独创性证据"]
        ...
        G["⑦ 全栈三层混合部署<br/>MySQL9 + Redis7 + ChromaDB<br/>MinIO + DeepSeek V4 Flash<br/>+ 阿里云百炼 Embedding<br/>真实环境适配与调试"]
    end
...
| ⑦ | **全栈真实环境混合部署** | `docker-compose.yml` + `config.py` + `application.yml` | ~200 | MySQL9 + Redis7 + ChromaDB + MinIO + DeepSeek V4 Flash(LLM) + 阿里云百炼(Embedding)的六组件协调部署设计 |
