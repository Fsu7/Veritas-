...
| **v0.4** | M4：多Agent协同完成 | Week 7-8（7/4 - 7/17） | 6个Agent协同+个性化引擎 | 15 | 🔄 代码就绪（2026-06-28 复验：6 Agent+graph.py 6 节点+SSE 端点全部实现，schemas 缺失阻断启动） |
| **v0.5** | M5：功能完整 | Week 9-10（7/18 - 7/31） | 所有P0+P1功能完成 | 16 | 🔄 进行中（2026-06-28 复验：JM5✅ 447 测试，JM5 后端缓存优化+导出已完成；前端 11 页+21 组件全部实现） |
...
| 序号 | 功能编号 | 功能名称 | 优先级 | 实现内容 | 产出文件 |
|------|---------|---------|--------|---------|---------|
| 1 | F3.1.1 | 协调者Agent | P0 | 任务分解与调度，汇总多Agent结果 | coordinator.py（538行）✅ |
| 2 | F3.1.4 | 对比Agent | P1 | 对比2-5篇论文，生成对比表格+矛盾检测 | comparer.py（795行）✅ |
| 3 | F3.1.6 | 审核Agent | P1 | 检测事实错误，核查引用 | reviewer.py（358行）✅ |
| 4 | F3.1.7 | 完整6-Agent工作流 | P0 | LangGraph编排：协调→检索→分析→[对比]→生成→审核 | graph.py（622行，6节点+条件边+重试）✅ |
| 5 | F3.1.8 | 降级机制 | P1 | 单Agent失败跳过，多Agent失败降级为单Agent模式 | graph.py + BaseAgent ✅ |

### 个性化引擎（F3.4）

| 序号 | 功能编号 | 功能名称 | 优先级 | 实现内容 | 产出文件 |
|------|---------|---------|--------|---------|---------|
| 6 | F3.4.2 | Prompt个性化 | P0 | 根据画像动态构建个性化Prompt片段 | personalization_service.py ✅（存在旁路：AppState 未实例化） |
| 7 | F3.4.3 | 内容难度适配 | P0 | 根据知识水平调整内容深度 | personalization_service.py DIFFICULTY_MAP ✅ |
| 8 | F3.4.4 | 风格适配 | P1 | 根据偏好风格调整表达方式 | personalization_service.py STYLE_MAP ✅ |

### 前端 — 多论文对比（F1.3）

| 序号 | 功能编号 | 功能名称 | 优先级 | 实现内容 | 产出文件 |
|------|---------|---------|--------|---------|---------|
| 9 | F1.3.4 | 多论文选择 | P1 | 勾选2-5篇论文进行对比 | PaperCard.vue（可选择）✅ |
| 10 | F1.3.5 | 对比分析展示 | P1 | 展示多论文对比表格 | CompareView.vue（450行）✅ |

### 前端 — 综述生成（F1.4）

| 序号 | 功能编号 | 功能名称 | 优先级 | 实现内容 | 产出文件 |
|------|---------|---------|--------|---------|---------|
| 11 | F1.4.1 | 综述生成页面 | P0 | 完整的综述生成流程 | ReportView.vue（444行）✅ |
| 12 | F1.4.2 | 个性化输出展示 | P0 | 展示不同画像的个性化结果 | ReportView.vue + Editor 三模式 ✅ |

### Java后端 — 分析服务扩展

| 序号 | 功能编号 | 功能名称 | 优先级 | API接口 | 产出文件 |
|------|---------|---------|--------|---------|---------|
| 13 | F2.4.2 | 对比分析请求 | P1 | `POST /api/analysis/compare` | AnalysisController.java ✅ |
| 14 | F2.4.3 | 综述生成请求 | P0 | `POST /api/analysis/report` | AnalysisController.java ✅ |
| 15 | F2.5.4 | SSE推送 | P1 | Agent状态实时推送到前端 | AgentController.java（420行 PythonAIClient）✅ |

### v0.4 验收检查点

```
✅ 协调者: 输入复杂问题，能分解为2-5个子任务（代码就绪）
✅ 对比: 输入3篇论文，生成对比表格，维度完整（代码就绪）
✅ 审核: 生成内容有事实错误时能检测出来（代码就绪）
✅ 工作流: 6-Agent顺序执行，条件分支正确（代码就绪，待 schemas 修复后实测）
✅ 降级: 人为制造Agent超时，系统不崩溃，返回降级结果（代码就绪）
⚠️ 个性化: 同一主题，本科生和博士生获得不同深度的综述（存在旁路：personalization_service 恒为 None）
✅ SSE: Agent执行过程中，前端能实时看到状态更新（代码就绪）
✅ 综述: 生成包含引言+现状+方法对比+趋势+参考文献的完整综述（代码就绪）
✅ 引用: 综述中引用标注格式正确（代码就绪）
□ 差异度: 同一主题不同画像差异度>60%（待 schemas 修复 + 个性化旁路修复后实测）
```
...
| 序号 | 功能编号 | 功能名称 | 优先级 | 实现内容 | 产出文件 |
|------|---------|---------|--------|---------|---------|
| 5 | F1.4.4 | PDF导出 | P1 | 导出PDF格式报告，引用保留 | PdfExporter.java + ExportPanel.vue ✅ |
| 6 | F1.4.4 | Word导出 | P1 | 导出Word格式报告 | WordExporter.java + ExportPanel.vue ✅ |
...
| 序号 | 功能编号 | 功能名称 | 优先级 | 实现内容 | 产出文件 |
|------|---------|---------|--------|---------|---------|
| 10 | F1.3.6 | 矛盾发现 | P2 | 自动标注论文间观点冲突 | ComparerAgent 内置 ✅ |
| 11 | F3.2.4 | 混合检索 | P1 | 关键词+语义检索，RRF融合 | search_service.py ✅ |

### Java后端 — 缓存优化（F2.6）

| 序号 | 功能编号 | 功能名称 | 优先级 | 实现内容 | 产出文件 |
|------|---------|---------|--------|---------|---------|
| 12 | F2.6.1 | 用户画像缓存 | P0 | Redis缓存画像，TTL 1h，更新时失效 | UserService @Cacheable ✅ |
| 13 | F2.6.2 | 检索结果缓存 | P1 | Redis缓存检索结果，TTL 10min | PaperService @Cacheable ✅ |
| 14 | F2.6.3 | 分析结果缓存 | P1 | Redis缓存分析结果，TTL 30min | AnalysisService @Cacheable ✅ |
| 15 | F4.2.3 | Agent状态缓存 | P1 | Redis缓存Agent执行状态 | AgentClientService Redis Hash ✅ |

### Python AI — 流式输出

| 序号 | 功能编号 | 功能名称 | 优先级 | 实现内容 | 产出文件 |
|------|---------|---------|--------|---------|---------|
| 16 | F3.3.3 | 流式输出 | P1 | LLM流式生成，首字节<2秒 | AgentOrchestrator + sse-starlette ✅（代码就绪，schemas 缺失阻断启动） |

### v0.5 验收检查点

```
✅ Agent流程图: 6个Agent节点正确显示，状态颜色正确（AgentFlowChart.vue 251行）
✅ Agent状态: SSE实时更新，等待/执行/完成/失败状态正确（AgentStatusPanel.vue 294行）
✅ 中间结果: 各Agent产出摘要正确展示（IntermediateResult.vue 155行）
✅ 耗时统计: 各Agent耗时柱状图正确（TimeStats.vue 180行）
✅ PDF导出: 导出文件格式正确，引用保留（ExportPanel.vue + PdfExporter）
✅ Word导出: 导出文件格式正确（ExportPanel.vue + WordExporter）
✅ 引用溯源: 点击引用可跳转到原文片段弹窗（CitationLink.vue 116行）
✅ 矛盾发现: 对比3篇以上论文时能发现观点矛盾（ComparerAgent + ReportView）
✅ 筛选: 年份/会议/引用数筛选正确（FilterPanel.vue 184行）
✅ 排序: 相关度/时间/引用排序正确（SortDropdown.vue 63行）
🔄 缓存: 命中率>50%（JM5 后端缓存优化已完成，待实测验证）
🔄 混合检索: 准确率>纯语义检索（search_service 已实现，待实测）
□ AnalyzerAgent 并行化（待 v1.0）
□ LLM Token Prometheus 监控（待 v1.0）
```
