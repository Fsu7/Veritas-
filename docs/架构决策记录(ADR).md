...
```
UserProfile (knowledge_level, preferred_style, research_field)
    ↓
PersonalizationService
    ├── KNOWLEDGE_MAP: {beginner, intermediate, advanced, expert}
    │     → 注入术语解释深度、方法论细节级别
    ├── STYLE_MAP: {simple, balanced, technical}
    │     → 注入行文风格、结构偏好
    └── research_field → 注入领域上下文关键词
    ↓
...
| 级别 | 术语处理 | 方法论 | 示例 |
|------|----------|--------|------|
| beginner | 全部解释 | 省略 | "简单来说，这篇论文研究的是..." |
...
| 缓存空间 | Key 模式 | TTL | 理由 |
|----------|----------|-----|------|
| 用户画像 | `user:profile:{userId}` | 1h | 画像低频变更 |
| 用户信息 | `user:info:{userId}` | 1h | 低频变更 |
| 论文详情 | `paper:detail:{paperId}` | 30min | 元数据极少变 |
| 论文列表 | `paper:list:{hash}` | 10min | 搜索结果短期有效 |
| 检索结果 | `search:result:{hash}` | 10min | RAG 检索成本高 |
| 会话状态 | `session:state:{sessionId}` | 2h | 会话期间保持 |
| Agent 状态 | `agent:state:{analysisId}` | 5min | 实时性要求高 |
| 分析结果 | `analysis:result:{analysisId}` | 30min | 已完成结果可缓存 |
| AI 降级状态 | `ai:provider:status` | 5min | 降级状态需及时感知 |
| JWT 黑名单 | `auth:token:blacklist:{tokenHash}` | Token有效期 | 登出即时失效 |
...
data: {"agent_name": "Analyzer", "status": "running", "progress": 0.6, "timestamp": "2026-05-23T10:30:00Z"}

event: agent_state_update
data: {"agent_name": "Analyzer", "status": "completed", "intermediate_result": "..."}
```
...
3. **创新点支撑**：知识图谱增强 RAG 是项目架构的重要补充能力，与纯 RAG 方案形成差异化
