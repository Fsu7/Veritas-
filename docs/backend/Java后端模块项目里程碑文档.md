...
| v1.2 | 2026-06-17 | 项目组 | **JM5 验收通过**：缓存优化与功能完善 11项交付物全部完成 ✅、12项验收检查点全部 ☑、JM5检查清单全部 ☑；task32-40 全部交付，95 个新增测试 + JM4 遗留 6 个测试修复，全量 447 测试通过；Cache-Aside 模式 + TTL ±10% 抖动防雪崩 + unless 空值过滤防穿透 + 精准 evict 防击穿三重防护；PDF 导出（iText 7 + STSong-Light）+ Word 导出（POI 5.2.3）+ 统一 export 入口；论文收藏 API（幂等 + @CacheEvict 精准失效）；论文检索扩展（author/keywords 过滤 + sortDirection 排序方向）；修复 String.format 与 SQL LIKE % 冲突；归档至 [log/backend/18_JM5缓存优化与功能完善/](file:///Users/achieve/Documents/AchiEVE_MacBook_Air/Veritas(求真)/log/backend/18_JM5缓存优化与功能完善/) |
...
| 序号 | 交付物 | 验收标准 | 状态 |
|------|--------|---------|------|
| 1 | 用户画像缓存 | @Cacheable + @CacheEvict，TTL 1h，命中率>50% | ✅ task32 完成（三重失效 userProfile+userProfileJson+userInfo） |
| 2 | 论文检索缓存 | 搜索结果缓存，TTL 10min，相同查询命中缓存 | ✅ task33/35 完成（9 参数复合 Key + null 规范化） |
| 3 | 分析结果缓存 | 分析结果缓存，TTL 30min | ✅ task34 完成（Spring Cache + 手动 RedisTemplate 双 Key + CacheManager evict） |
| 4 | 会话状态缓存 | 会话状态缓存，TTL 2h | ✅ task34 完成（sessionState TTL=2h + sessionList TTL=10min） |
| 5 | Agent状态缓存完善 | Hash结构，5min TTL，与SSE同步 | ✅ task34 完成（agentState Hash + 5min TTL） |
| 6 | 论文筛选排序 | 年份/会议/引用数筛选 + 相关度/时间/引用排序 | ✅ task35 完成（新增 author/keywords 过滤 + sortDirection + 9 测试） |
| 7 | 论文收藏API | POST/DELETE /api/papers/{paperId}/favorite | ✅ task36 完成（幂等性 + @CacheEvict 精准失效 + 11 测试） |
| 8 | 收藏列表API | GET /api/papers/favorites 分页查询 | ✅ task36 完成（@Cacheable + 批量查 Paper 避免 N+1） |
| 9 | PDF导出服务 | PdfExporter生成格式正确的PDF | ✅ task37 完成（iText 7 + STSong-Light 中文字体 + Markdown + 页脚 + 8 测试） |
| 10 | Word导出服务 | WordExporter生成格式正确的Word文档 | ✅ task38 完成（POI 5.2.3 + 宋体 + 统一 export 入口 + 12 测试） |
| 11 | 缓存一致性保障 | Cache-Aside写后删+双重失效（画像更新） | ✅ task39 完成（32 个缓存测试：命中率 + 一致性 + 防穿透/防雪崩） |
...
```
☑ 画像缓存: 更新画像后缓存立即失效（三重失效 userProfile+userProfileJson+userInfo）
☑ 检索缓存: 相同查询第二次直接返回缓存（9 参数复合 Key + null 规范化）
☑ 分析缓存: 已完成分析结果可缓存命中（Spring Cache + 手动 RedisTemplate 双 Key）
☑ 缓存命中率: >50%（TTL 分层 + 抖动防雪崩 + 空值过滤防穿透）
☑ 筛选: 年份范围/会议/引用数/作者/关键词筛选正确（task35 扩展）
☑ 排序: 相关度/时间/引用排序正确（sortDirection asc/desc，非法值 fallback desc）
☑ 收藏: 收藏/取消收藏操作正确（幂等性 + @CacheEvict 精准失效）
☑ 收藏列表: 分页查询用户收藏论文（@Cacheable + 批量查 Paper 避免 N+1）
☑ PDF导出: 格式正确，引用保留（iText 7 + STSong-Light + Markdown 渲染 + 页脚）
☑ Word导出: 格式正确，可编辑（POI 5.2.3 + 宋体 + 统一 export 入口）
☑ Cache-Aside: 写操作后缓存失效，读操作回填（@Cacheable + @CacheEvict）
☑ 双重失效: 画像更新时userProfile+userProfileJson+userInfo同时失效（三重 @CacheEvict）
```
...
| **交付物** | 11 项全部完成 ✅ |
| **验收检查点** | 12 项全部通过 ☑ |
| **新增测试** | 95 个（task35:9 + task36:11 + task37:11 + task38:24 + task39:32 + task40:8） |
| **全量测试** | 447 个全部通过，0 失败 0 错误 |
| **JM4 遗留修复** | 6 个测试（RedisKeyUtilTest 1 + PaperControllerTest 3 + PaperServiceCacheTest 2） |
| **关键修复** | String.format 与 SQL LIKE % 冲突、PdfExporter 字体加载、POI HeaderFooterType 包路径 |
| **缓存防护** | TTL ±10% 抖动防雪崩 + unless=#result==null 防穿透 + key=#userId 精准 evict 防击穿 |
| **归档位置** | [log/backend/18_JM5缓存优化与功能完善/](file:///Users/achieve/Documents/AchiEVE_MacBook_Air/Veritas(求真)/log/backend/18_JM5缓存优化与功能完善/) |
