...
graph LR
    ...
    style FM1 fill:#e3f2fd,stroke:#1565c0
    style FM2 fill:#e8f5e9,stroke:#2e7d32
    style FM3 fill:#fff3e0,stroke:#ef6c00
    style FM4 fill:#fce4ec,stroke:#c62828
    style FM5 fill:#f3e5f5,stroke:#6a1b9a
    style FM6 fill:#e0f7fa,stroke:#00695c
```

> **说明**：前端在M2（Week 3-4）阶段无专属任务，该阶段主要开发Python AI服务。前端开发集中在M1、M3-M6。FM2（用户+检索页面）是前端最关键的里程碑，决定了用户能否通过界面使用基本功能。
...
| 4 | Vite配置 | Element Plus自动按需导入 + ECharts按需导入 + manualChunks分包 | ⬜ |
| 5 | 环境变量配置 | .env + .env.development + .env.production，VITE_API_BASE_URL配置正确 | ⬜ |
| 6 | Axios实例+拦截器 | 请求拦截器注入JWT Token + 响应拦截器统一错误处理（401/403/404/超时） | ⬜ |
| 7 | Vue Router配置 | 9条路由定义（懒加载） + requiresAuth元数据 + 全局前置守卫 | ⬜ |
| 8 | Pinia Store骨架 | userStore + paperStore + sessionStore + agentStore 基础结构 | ⬜ |
| 9 | TypeScript类型定义 | paper.ts + user.ts + analysis.ts + agent.ts + common.ts 完整类型 | ⬜ |
| 10 | 全局样式 | variables.scss（CSS变量+Agent状态色）+ global.scss | ⬜ |
| 11 | AppHeader布局组件 | 顶部导航栏（Logo+菜单+用户信息+退出） | ⬜ |
| 12 | HomeView首页骨架 | 基础布局，主题输入框占位 | ⬜ |
...
| 5 | userStore完整实现 | login/logout/fetchProfile/saveProfile + Token管理 | ⬜ |
| 6 | useAuth鉴权组合函数 | 登录状态判断 + 自动跳转 + Token过期处理 | ⬜ |
| 7 | HomeView首页完整 | 主题输入框 + 历史搜索标签 + 检索触发 | ⬜ |
| 8 | SearchView检索结果页 | 论文卡片列表 + 分页 + 加载状态 | ⬜ |
| 9 | PaperCard论文卡片组件 | 标题/作者/摘要/关键词/相关度/推荐理由/操作按钮 | ⬜ |
| 10 | paperStore完整实现 | searchPapers/togglePaperSelection/toggleFavorite | ⬜ |
| 11 | PaperDetailView论文详情页 | 论文元数据展示 + 触发AI分析按钮 | ⬜ |
| 12 | AnalysisCard分析卡片（基础） | 5维度结构化展示（研究问题/方法/实验/结论/局限） | ⬜ |
| 13 | sessionStore基础实现 | createSession/fetchAnalysisResult | ⬜ |
...
| 2 | PaperSelector论文选择器 | 勾选2-5篇论文，已选标签展示，超限提示 | ⬜ |
| 3 | CompareView对比分析页 | 对比表格展示 + 矛盾发现标注 | ⬜ |
| 4 | CompareTable对比表格组件 | 维度×论文矩阵 + 差异高亮 + 矛盾告警 | ⬜ |
| 5 | ReportView综述生成页（基础） | 综述内容展示 + 画像信息 + 生成时间 | ⬜ |
| 6 | ReportPreview综述预览组件 | Markdown渲染 + 引用标注可点击 | ⬜ |
| 7 | markdown-it渲染工具 | Markdown→HTML + 引用链接自定义渲染 | ⬜ |
| 8 | agentStore完整实现 | updateAgentState/getAgentState/resetStates/progress | ⬜ |
| 9 | useSSE组合函数 | SSE连接/断开/自动重连（3s间隔，最多5次） | ⬜ |
...
| FM3 | SSE连接不稳定 | 中 | Agent状态不更新 | useSSE自动重连（3s间隔，最多5次） |
| FM3 | Markdown渲染XSS风险 | 低 | 安全漏洞 | markdown-it配置html:false |
| FM3 | 对比表格列数过多 | 低 | 布局错乱 | 横向滚动+固定首列 |
...
□ vite.config.ts ECharts按需导入
□ vite.config.ts manualChunks分包配置
□ .env + .env.development + .env.production配置
...
□ userStore login/logout/fetchProfile/saveProfile
□ useAuth鉴权组合函数
□ HomeView首页主题输入+历史搜索
□ SearchView检索结果页+分页
□ PaperCard论文卡片组件
□ paperStore searchPapers/togglePaperSelection
□ PaperDetailView论文详情页
□ AnalysisCard分析卡片5维度
□ sessionStore createSession/fetchAnalysisResult
□ usePagination分页组合函数
□ 全链路: 注册→登录→画像→检索→详情→分析 无报错
...
□ PaperSelector论文选择器（2-5篇）
□ CompareView对比分析页
□ CompareTable对比表格+矛盾告警
□ ReportView综述生成页（基础）
□ ReportPreview Markdown渲染
□ markdown-it渲染工具（引用链接自定义）
□ agentStore完整实现
□ useSSE组合函数（连接/断开/重连）
...
| 里程碑 | 新增文件 | 累计文件数 |
|--------|---------|-----------|
| FM1 | main.ts + App.vue + 4config + 4store骨架 + 5types + 5api骨架 + router + 2composable骨架 + 2styles + 3layout组件 + HomeView骨架 | ~30 |
...
| FM3 | 2views(Compare/Report) + 4components(PlainExplanation/PaperSelector/CompareTable/ReportPreview) + 1composable(useSSE) + 1util(markdown) | ~58 |
