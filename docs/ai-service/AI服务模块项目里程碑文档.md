...
| 5 | EmbeddingService | text-embedding-v4 API调用成功，返回768维向量 | ⬜ |
...
| 风险 | 应对 |
|------|------|
| 阿里云百炼API不可用 | 检查DASHSCOPE_API_KEY配置；备选本地bge-large-zh模型 |
...
□ EmbeddingService: text-embedding-v4 API调用成功，768维向量
□ 批量向量化: 100条文本<10秒
□ LLMService: 至少一路模型可用
