...
| F5.2 | Embedding模型模块 | 文本向量化(bge-large-zh)、批量处理 | P0 |
...
| 5 | EmbeddingService | bge-large-zh-v1.5加载成功，`encode("测试")` 返回768维向量 | ⬜ |
...
| 天数 | 任务 | 产出 |
|------|------|------|
| Day 1 | bge-large-zh模型下载 + EmbeddingService | services/embedding_service.py |
...
□ 批量向量化: 100条文本耗时<10秒
□ ChromaDB: collection.count() 返回非负数
□ LLM方案A: 软件方模型调用成功，返回正确文本
□ LLM方案B: 外接API调用成功（需API Key）
□ LLM方案C: 本地模型加载成功（需GPU或CPU模型）
□ LLM降级: 软件方不可用时自动切换到外接API
□ Prompt模板: 模板文件加载成功，变量替换正确
□ Pydantic: 请求模型校验生效（空topic返回422）
□ 异常处理: 自定义异常返回统一JSON格式
□ 日志: 控制台+文件双输出，格式正确
...
| 风险 | 应对 |
|------|------|
| bge-large-zh下载慢/失败 | 使用HuggingFace镜像站或提前下载模型文件 |
| GPU不可用 | 使用CPU模式运行，或仅配置API方案 |
| 软件方模型不可用 | 立即切换到外接API方案，确保至少一路可用 |
| Python依赖冲突 | 使用虚拟环境，固定版本号 |
...
| 里程碑 | 关键风险 | 概率 | 影响 | 应对措施 |
|--------|---------|------|------|---------|
| AM1 | bge-large-zh下载/加载失败 | 中 | AM2延迟 | 提前下载模型；使用镜像站；备选Embedding API |
...
| 时间 | 必须完成的风险应对 |
|------|------------------|
| **AM1前** | 确认Python 3.10+环境可用；提前下载bge-large-zh模型 |
...
| 里程碑 | 核心验收标准 | 不通过条件 |
|--------|-------------|-----------|
| AM1 | FastAPI可启动，至少一路LLM可用，Embedding 768维向量 | 模型不可用 |
...
□ EmbeddingService: bge-large-zh加载成功，768维向量
□ 批量向量化: 100条文本<10秒
□ LLMService: 至少一路模型可用
□ BuiltinLLMProvider: 软件方模型连接测试
□ APILLMProvider: 外接API配置就绪
□ LocalLLMProvider: 本地模型框架就绪
□ LLM自动降级: 软件方→外接API→本地模型切换正常
□ VectorStoreService: ChromaDB PersistentClient初始化
□ Prompt模板: 目录结构就绪，加载函数可用
□ Pydantic模型: 请求响应模型定义完整
□ 异常体系: AIServiceException + 全局处理器
□ 日志: Loguru控制台+文件双输出
□ Dockerfile构建成功
□ .env.example配置完整
```
