...
| TypeScript | 6.0+ | 类型安全 |
| Vite | 8.0+ | 构建工具 + 开发服务器 |
| Pinia | 2.3+ | 状态管理（Setup Store 风格） |
| Vue Router | 4.6+ | 路由管理（懒加载 + 导航守卫） |
| Element Plus | 2.14+ | UI 组件库 |
| ECharts | 5.6+ | 图表可视化（Agent 流程图等） |
| Axios | 1.16+ | HTTP 客户端（统一拦截器） |
| markdown-it | 14.2+ | Markdown 渲染（综述报告展示） |
| Sass | 1.100+ | CSS 预处理器 |
| Vitest | 3.2+ | 单元测试框架 |
...
| `env.d.ts` | Vite 环境类型声明，让 TypeScript 识别 `import.meta.env` 等 Vite 特有变量 |
| `auto-imports.d.ts` | 由 `unplugin-auto-import` 自动生成的类型声明，包含 Vue / Vue Router / Pinia 的自动导入类型 |
| `components.d.ts` | 由 `unplugin-vue-components` 自动生成的组件类型声明，配合 Element Plus 按需导入 |
...
| 文件 | 说明 |
|------|------|
| `index.ts` | **Axios 实例封装**。基础配置：`baseURL` 从 `VITE_API_BASE_URL` 读取（默认 `/api`），超时 30s，JSON 格式。**请求拦截器**：从 `userStore` 读取 JWT Token 注入 `Authorization` 头。**响应拦截器**：成功时解包 `ApiResponse.data`；失败时按状态码统一处理（401→清空登录态跳转登录页、403→无权限提示、404→资源不存在、ECONNABORTED→超时提示、其他→网络错误提示），所有错误通过 `ElMessage` 弹出 |
...
Vue 3 Composition API 的可复用逻辑单元（习惯上以 `use` 前缀命名），如 `useSSE`（SSE 事件流封装）、`usePagination`（分页逻辑）等。当前通过 `.gitkeep` 占位，待开发。
...
| 文件 | Store 名 | 说明 |
|------|---------|------|
| `userStore.ts` | `user` | **用户认证与画像管理**。状态：`token`/`userId`/`username`（同步至 localStorage）、`profile`（UserProfile）。计算属性：`isLoggedIn`/`hasProfile`。方法：`setLoginData`（登录数据持久化）、`logout`（清空登录态）、`fetchProfile`/`saveProfile`（TODO：API 调用） |
| `paperStore.ts` | `paper` | **论文搜索与选择管理**。状态：`searchResults`（搜索结果列表）、`selectedPapers`（已选论文，上限5篇）、`favorites`（收藏ID列表）、`filters`/`currentQuery`/`totalResults`/`currentPage`/`pageSize`。计算属性：`selectedPaperIds`/`filteredResults`。方法：`searchPapers`（TODO）、`togglePaperSelection`（选择/取消，上限5篇）、`toggleFavorite`（TODO） |
| `agentStore.ts` | `agent` | **Agent 执行状态管理**。状态：`agentStates`（6个 Agent 状态映射）、`flowData`（流程图数据）、`isConnected`（SSE 连接状态）、`currentAnalysisId`。计算属性：`agentStatesList`/`activeAgents`（运行中的 Agent）/`progress`（完成进度 0~1）。方法：`getAgentState`/`updateAgentState`/`resetStates` |
| `sessionStore.ts` | `session` | **分析会话管理**。状态：`currentSessionId`/`currentAnalysisId`/`analysisResults`（Map 结构缓存）。方法：`createSession`/`fetchAnalysisResult`（TODO） |
...
| `assets/vendor-*.js` | Vue/VueRouter/Pinia/Axios（约30KB） |
| `assets/element-plus-*.js` | Element Plus 按需导入（约196KB） |
| `assets/element-plus-*.css` | Element Plus 样式（约55KB） |
...
| 文件 | 说明 |
|------|------|
| `package.json` | 项目元数据（`literature-assistant-frontend` v0.1.0，ESM 模式）。脚本：`dev`(开发服务器)/`build`(类型检查+构建)/`preview`(预览构建产物)/`typecheck`(类型检查)/`test`(Vitest 单元测试)/`test:run`(单次运行测试) |
...
| `tsconfig.json` | TypeScript 项目配置，引用 `tsconfig.node.json` |
...
| `.env` | 通用环境变量：`VITE_APP_TITLE=科研文献智能助手` |
...
| API 调用 | 统一通过 `src/api/index.ts` 的 Axios 实例，不直接使用原生 axios |
...
| API 封装 | ✅ 已完成 | Axios 拦截器 + JWT 注入 + 统一错误处理 |
| 类型体系 | ✅ 已完成 | 5 个类型文件覆盖全部业务域 |
| Store 层 | 🔧 骨架完成 | 4 个 Store 已定义状态/计算属性，API 调用方法标注 TODO |
| 路由守卫 | ✅ 已完成 | 认证检查 + 登录重定向 |
| 构建优化 | ✅ 已完成 | 三分包（Element Plus / ECharts / Vendor） |
| Docker 部署 | ✅ 已完成 | 多阶段构建 + Nginx + 健康检查 |
| 业务页面 | ⏳ 待开发 | 8 个页面中仅首页已实现，其余为占位符 |
| 业务组件 | ⏳ 待开发 | 6 个组件目录均为空 |
| 组合函数 | ⏳ 待开发 | composables/ 为空 |
| 工具函数 | ⏳ 待开发 | utils/ 为空 |
